...
void iec_unlinkIO(iec_device *device, iec_io *io, int inout)
{
  iec_chain *chain;
  iec_io *last;
...
static void iec_sendData(struct work_struct *work)
{
  ...
  printk(KERN_NOTICE "IEC: talker %i\n", abort);

  while (iec_state == IECOutputState && !abort) {
    printk(KERN_NOTICE "IEC: sendData 1\n");
    if (!(device = iec_openDevices[iec_curDevice]))
      return;
    if (!device->out.head)
      return;

    printk(KERN_NOTICE "IEC: sendData 2 - %i\n", iec_curChannel);
    io = device->out.head;
    while (io && io != device->out.cur && io->header.channel != iec_curChannel) {
      printk(KERN_NOTICE "IEC: channel %i\n", io->header.channel);
      io = io->next;
    }
    ...
    printk(KERN_NOTICE "IEC: sendData 3\n");
    data = io->data;
    ...
    printk(KERN_NOTICE "IEC: sendData 6\n");
  if (abort) {
    iec_state = IECWaitState;
    iec_releaseBus();
  }
...
ssize_t iec_read(struct file *filp, char *buf, size_t count, loff_t *f_pos)
{
  ...
  printk(KERN_NOTICE "IEC: want %i bytes\n", count);
  do {
    io = device->in.head;
    if (io && io != device->in.cur) {
      avail = (io->header.len + 4) - io->pos;
  ...
  count -= remaining;
  io->pos += count;
  printk(KERN_NOTICE "Sent %i bytes\n", count);
